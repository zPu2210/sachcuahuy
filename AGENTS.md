...
ALWAYS activate `Codex` skill before starting any implementation.
...
```
/Users/duynguyen/www/claudekit-marketing/.Codex
```

When current user is "bnqtoan" (macOS):
```
/Applications/MAMP/htdocs/claudekit-marketing/claudekit-marketing/.Codex
```

When current user is "admin" (Windows):
```
D:\www\claudekit\claudekit-marketing\.Codex
```

**Structure:**
```
claudekit-marketing/
├── .Codex/                    ← KIT OUTPUT (what we're building)
...
**REMEMBER:**
- **All new skills, agents, commands → go to `.Codex/`**
...
ClaudeKit Marketing is a comprehensive toolkit for sales and marketing automation using Codex subagent orchestration. This toolkit enables autonomous marketing workflows for content creation, campaign management, SEO optimization, and lead generation.

**Target Users:** Indie hackers, small marketing teams, SMB marketing managers

## What We're Building (CRITICAL)

**The deliverable is the `.Codex/` folder itself** - a reusable kit users install in THEIR projects.

```
OUTPUT: ./claudekit-marketing/.Codex/
        ├── agents/      # Marketing subagents
...
**Script:** `.Codex/skills/brand-guidelines/scripts/inject-brand-context.cjs`
