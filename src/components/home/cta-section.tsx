      ...
      <div className="container-custom relative z-10 text-center">
        ...
        <FadeIn delay={0.1}>
          <h2 className="font-serif text-4xl md:text-5xl md:leading-tight font-bold text-white mb-6">
            ...
            <span className="text-accent">Miền Nam của Huy</span> ngay hôm nay
          </h2>
        </FadeIn>

        <FadeIn delay={0.2}>
          <p className="text-white/70 mb-10 max-w-xl mx-auto text-lg">
            Đặt hàng ngay hôm nay để nhận được ấn bản đầu tiên với <strong className="text-white">chữ ký tác giả</strong> và <strong className="text-white">bookmarks</strong> độc quyền.
          </p>
        </FadeIn>

        <FadeIn delay={0.3}>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href={`/sach/${featuredBook.slug}`} className="w-full sm:w-auto btn bg-white text-primary hover:bg-accent hover:text-white border-0 px-8 py-4 text-lg font-bold shadow-xl shadow-black/20">
              Đặt Hàng Ngay
              <ArrowRight className="w-5 h-5 ml-2" />
            </Link>
            <span className="text-white/50 text-sm hidden sm:inline-block">hoặc</span>
            <Link href="/sach" className="w-full sm:w-auto btn border border-white/30 text-white hover:bg-white/10 px-8 py-4">
              Xem Thêm Tác Phẩm
            </Link>
          </div>
